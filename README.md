...
layers before the auto-merge workflow can merge a pull request.

## Delivery policy

All changes are made on a branch and delivered by pull request. The CI workflow is the required
quality gate. Successful internal, non-draft pull requests are squash-merged by the auto-merge
workflow; repository branch protection should also require the `CI / Python quality and tests`
and `CI / Playwright end-to-end` checks.
